...
@react class App extends StatelessComponent {
  ...
  private val examples: List[(String, String)] = List(
    ...
    "yCombinator x"                                               -> "the keyword `yCombinator` allows for recursion, duplicating the expression on the righthand and infinitely replacing it on the lefthand side",
    "yCombinator (fac . n . ( (<= n 1) 1 (* n (fac (- n 1))))) 3" -> "yCombinator with the non-recursive part as an argument"
  )
